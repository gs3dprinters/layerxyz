...
export default function ShopByCategory() {
  ...
  const activeProducts = getProductsByCategory(activeCategory.slug);
  const activeCount = getCategoryProductCount(activeCategory.slug);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-start">
              {/* Left Column: Category Narrative & Subcategories */}
              <div className="lg:col-span-5 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-xs font-mono tracking-wider text-[#6F6B63]">
                      {activeCount > 0 ? `${activeCount} ${activeCount === 1 ? 'OBJECT' : 'OBJECTS'}` : 'MADE TO ORDER'}
                    </span>
                  </div>
                  ...
                  <p className="text-sm sm:text-base text-[#5A5750] leading-relaxed mb-6">
                  ...
                  <div className="mb-6 pt-5 border-t border-[#E8E5DE]">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {activeCategory.subcategories.map((sub) => (
                        <Link
                          key={sub.slug}
                          href={`/categories/${activeCategory.slug}?subcategory=${sub.slug}`}
                          className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-white text-[#171716] border border-[#E8E5DE] hover:border-[#171716]/40 transition-colors shadow-xs"
                        >
                          {sub.name}
                        </Link>
                      ))}
                ...
                <div className="flex flex-wrap items-center gap-3 pt-4">
                  ...
                  <Link
                    href={`/custom?category=${activeCategory.slug}`}
              ...
              {/* Right Column: Real Products or Empty State */}
              <div className="lg:col-span-7 flex flex-col">
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-[#E8E5DE]">
                  <span className="text-[11px] font-mono tracking-wider uppercase text-[#6F6B63] font-medium">
                    FEATURED IN {activeCategory.name.toUpperCase()}
                  </span>
                  <Link
                    href={`/categories/${activeCategory.slug}`}
                ...
                {activeProducts.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {activeProducts.slice(0, 4).map((product) => (
                      ...
                      >
                        <div>
                          {product.images?.[0] && (
                            <div className="relative aspect-[4/3] w-full overflow-hidden rounded-xl bg-[#FAFAF8] border border-[#E8E5DE] mb-3">
                              <img
                          ...
                          <h4 className="text-base font-semibold text-[#171716] group-hover:text-black transition-colors mb-1 line-clamp-1">
                            {product.name}
                          </h4>

                          <p className="text-xs text-[#6F6B63] line-clamp-2 leading-relaxed mb-3">
                            {product.description}
                          </p>
                        </div>
                        ...
                        <div className="pt-3 border-t border-[#F0ECE4] flex items-center justify-between">
                          ...
                          <span className="inline-flex items-center gap-1 text-xs font-mono uppercase text-[#171716] group-hover:translate-x-0.5 transition-transform">
                            VIEW OBJECT
                            <ArrowRight size={12} />
                          </span>
                        </div>
                  ...
                  <div className="rounded-2xl bg-white border border-[#E8E5DE] p-8 sm:p-10 text-center">
                    ...
                    <Link
                      href={`/custom?category=${activeCategory.slug}`}
                      className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-full bg-[#171716] text-[#F4F1EA] text-xs font-mono uppercase tracking-wider hover:bg-[#2A2A28] transition-colors font-medium"
                    >
                      COMMISSION A PIECE
                      <ArrowRight size={12} />
                    </Link>
        ...
        <div className="mt-8">
          <div className="mb-6 flex items-center justify-between">
            ...
            <Link
              href="/categories"
              className="text-xs font-mono uppercase text-[#171716] hover:underline"
            >
              VIEW ALL →
            </Link>
          </div>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div className="space-y-1 mb-4">
                      {cat.subcategories.slice(0, 3).map((sub) => (
                        <div key={sub.slug} className="text-[11px] text-[#55524B] flex items-center gap-1.5">
                  ...
                  <div className="pt-3 border-t border-[#E8E5DE] flex items-center justify-between text-xs">
                    <span className="text-[10px] font-mono uppercase text-[#8E8B83]">
                      {count > 0 ? `${count} Objects` : 'Made to Order'}
                    </span>
                    <span className="font-mono text-xs text-[#171716] font-medium group-hover:underline">
                      Explore →
                    </span>
                  </div>
                </Link>
